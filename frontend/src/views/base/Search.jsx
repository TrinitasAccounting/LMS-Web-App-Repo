...
import apiInstance from '../../utils/axios';           // I think we can use useAxios and apiInstance interchangeably. apiInstance might actually be the non secured fetch method
...
function Search() {
  ...
  const addToCart = async (courseId, userId, price, country, cartId) => {
    ...
    try {
      await useAxios().post(`course/cart/`, formdata).then((res) => {
        ...
        //Set Cart count after addign to cart, basically just fetching all of the items in that cart again and counting the length of it
  ...
  const handleSearch = (event) => {
    ...
    // THIS DOES NOT WORK PROPERLY AS WHEN ONE BACK SPACES WE ARE HAVING TO REMOVE EVERYTHING
    // Instead we should be passing this filtered "course" array down to be mapped through, but we would need to set the if part as const filteredCourses =
    else {
          ...
          <div className="row">
            <div className="col-md-12">
              <div className="row row-cols-1 row-cols-md-2 row-cols-lg-4 g-4">
                ...
                {/* SHOWING THE COURSES AVAILABLE BY MAPPING THROUGH COURSES */}
                {courses?.map((c, index) => {
                  return (
                    <div className="col">
                      {/* Card */}
